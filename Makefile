...
run:
	...
	appc reg -n appweb -e APP_DOCKER_OPTS="--net=host -v /opt/appmgr/ssl/server.pem:/etc/nginx/conf.d/server.crt:ro -v /opt/appmgr/ssl/server-key.pem:/etc/nginx/conf.d/server.key:ro" -c "nginx -g 'daemon off;'" -d ${DOCKER_IMG_NAME} -f
tar:
