  ...
  <el-card class="box-card" v-if="record">
    ...
    <DescriptionList title="Runtime infomation" col="12">
      ...
      <Description term="Last start time">
        <span
          v-if="record.last_start_time"
        >{{ record.last_start_time | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        <span v-else>-</span>
      </Description>
    ...
    <DescriptionList title="Extra infomation" col="12">
      <Description
        term="Daily limitation"
      >{{(record.daily_limitation) ? record.daily_limitation.daily_start + ' - ' + record.daily_limitation.daily_end : '-'}}</Description>
      <Description term="Posix timezone">{{record.posix_timezone | formatEmpty}}</Description>
      <Description
        term="Phisical memory (Mi)"
      >{{record.resource_limit? record.resource_limit.memory_mb:'-'}}</Description>
      <Description
        term="Virtual memory (Mi)"
      >{{record.resource_limit? record.resource_limit.memory_virt_mb:'-'}}</Description>
      <Description term="CPU shares">{{record.resource_limit? record.resource_limit.cpu_shares:'-'}}</Description>
      <Description term="stdout cache size">{{record.stdout_cache_size | formatEmpty}}</Description>
    </DescriptionList>
    <el-divider></el-divider>
