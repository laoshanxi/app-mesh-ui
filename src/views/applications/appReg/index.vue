    ...
    <el-card shadow="never" class="register-card">
      <el-form :model="registerForm" ref="regForm" :rules="regRules" label-width="160px">
        ...
        <el-form-item label="Daily limitation">
          <el-time-picker is-range v-model="daily_time_range" range-separator="-" value-format="HH:mm:ss"
            start-placeholder="Start time" end-placeholder="End time"></el-time-picker>
        </el-form-item>
        <el-divider></el-divider>
...
export default {
  ...
  methods: {
    ...
    registerApp() {
      ...
      if (this.registerForm.daily_start_TEXT && this.registerForm.daily_start_TEXT !== "") {
        // convert day time "08:00:00" with current browser zone to UTC seconds
        let fullDateString = `1970-01-01 ${this.registerForm.daily_start_TEXT}`; // Combine with time (no 'Z' for local time)
        this.registerForm.daily_start = Date.parse(fullDateString) / 1000; // Convert to seconds
      }
      if (this.registerForm.daily_end_TEXT && this.registerForm.daily_end_TEXT !== "") {
        // convert day time "08:00:00" with current browser zone to UTC seconds
        let fullDateString = `1970-01-01 ${this.registerForm.daily_end_TEXT}`; // Combine with time (no 'Z' for local time)
        this.registerForm.daily_end = Date.parse(fullDateString) / 1000; // Convert to seconds
      }
